...
SELECT Title, First_Name || ' ' || Last_Name AS Author, Published_Year, Genre, Available_Copies
FROM Books 
...
SELECT First_Name || ' ' || Last_Name AS Member, COUNT(Borrow_id) AS Total_Borrow 
FROM Borrowed_Books
JOIN Members ON Members.Member_id=Borrowed_Books.Member_Id
GROUP BY Member
ORDER BY Total_Borrow DESC
...
SELECT First_Name || ' ' || Last_Name AS Member, Email, Phone_Number, COUNT(Borrowed_Books.Member_id) AS NO_of_Books_Borrowed,
CASE
...
FROM Borrowed_Books
JOIN Members ON Members.Member_id=Borrowed_Books.Member_Id
GROUP BY Member, Email, Phone_Number;